"""
Snake Game Online — نسخة Streamlit (لعب جماعي بدون عنوان IP)
=====================================================
نسخة من لعبة الثعبان تعمل داخل المتصفح عبر Streamlit. كل لاعب يفتح نفس
رابط التطبيق، يكتب اسم مستخدم فقط، ويبدأ اللعب مباشرة — لوحة الصدارة
مشتركة تلقائيًا بين الجميع دون الحاجة لإدخال أي عنوان IP يدويًا.

كيف تتم مشاركة لوحة الصدارة تقنيًا (خلف الكواليس):
    التطبيق يتصل بخادم لوحة صدارة صغير (server.py) عبر عنوان ثابت محدَّد
    مسبقًا داخل الكود (SERVER_URL أدناه). بما أن كل من يفتح نفس رابط
    Streamlit يتصل بنفس هذا العنوان تلقائيًا، لا يحتاج أي لاعب لمعرفة أو
    كتابة أي عنوان IP بنفسه.

المتطلبات:
    pip install streamlit requests flask

التشغيل (على جهاز واحد، أو خادم استضافة واحد):
    1. شغّل خادم لوحة الصدارة مرة واحدة:
           python server.py
    2. شغّل تطبيق اللعبة:
           streamlit run streamlit_app.py
    3. أرسل رابط تطبيق Streamlit فقط لأصدقائك (لا حاجة لإرسال أي IP) —
       كل من يفتحه يكتب اسمه ويلعب، وتظهر نتيجته للجميع تلقائيًا.

ملاحظة للمطوّر / من يستضيف التطبيق:
    القيمة الافتراضية لـ SERVER_URL بالأسفل هي "http://127.0.0.1:5000"
    (تعمل عندما يشتغل server.py على نفس الجهاز الذي يعمل عليه Streamlit).
    إذا كنت تنشر التطبيق على الإنترنت لأشخاص من أماكن مختلفة، استضف
    server.py على خدمة مثل Render أو Railway، ثم غيّر قيمة SERVER_URL
    بالأسفل (أو عرّف متغير بيئة SNAKE_SERVER_URL) لتشير لرابط ذلك الخادم.
    هذه الخطوة تُنفَّذ مرة واحدة فقط من طرفك، ولا يراها أو يتعامل معها
    اللاعبون إطلاقًا.
...
# عنوان خادم لوحة الصدارة — ثابت، لا يظهر للاعبين ولا يُطلب منهم إدخاله.
...
st.caption(
    "اكتب اسم مستخدم داخل مربع اللعبة وابدأ اللعب مباشرة. النتائج تُشارك "
    "تلقائيًا مع كل من يفتح هذا التطبيق. استخدم أسهم الكيبورد للتحريك، "
    "ومسافة للإيقاف المؤقت — اضغط داخل مربع اللعبة أولًا لتفعيل الأسهم."
)
...
with st.sidebar:
    ...
    try:
        ...
        st.caption("⚠️ تعذر الاتصال بخادم لوحة الصدارة. اللعبة تعمل محليًا بدون مشاركة النتائج حاليًا.")
